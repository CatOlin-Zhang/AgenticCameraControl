...
| `tracking` | int | 0/1 | human, vehicle | Enable/disable auto-tracking (area does not support tracking) |
| `level` | int | 0–3 | all | Sensitivity level: 0=off, 1=low, 2=medium, 3=high |
| `indoor` | int | 0–2 | human | Scene mode: 0=outdoor, 1=indoor, 2=outdoor-person-vehicle |
| `screenenable` | int | 0/1 | human | Target marker overlay on video |
| `blink` | int | 0/1 | human | Bounding-box blink effect |
| `drag` | int | 0/1 | vehicle | Target marker display |
| `alarm_enable` | int | 0/1 | human, vehicle | Audio alarm on detection |
| `alarm_type` | int | — | human, vehicle | Alarm type selector |
| `white_light` | int | 0/1 | human, vehicle | White-light alarm on detection |
| `duration` | int | — | human, vehicle | White-light duration (seconds) |
| `timestrategy` | int/string | — | human, vehicle | Detection schedule |
| `humandistance` | int | — | human | Min human size threshold |
| `thresh` | int | — | area | Sensitivity threshold |
...
| Value | Scope |
|-------|-------|
| `all` | Query all three types (default) |
...
| `tracking` | bool | Enable/disable auto-tracking (human/vehicle only; ignored for area) |
...
| `camera` | string | Camera name |
| `channel` | string | Always `"sk"` |
| `detect_type` | string | Which detection type was modified (`human`/`vehicle`/`area`) |
...
| `INVALID_DETECT_TYPE` | `detect_type` is not `human`, `vehicle`, or `area` |
