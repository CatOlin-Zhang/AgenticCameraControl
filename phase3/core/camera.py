...
from phase3.core.events import EventBus, CameraEvent, EventType, StubOnvifEventListener
...
class ONVIFCameraConnection:
    ...
    def _connect_rtsp_only(self) -> bool:
        """RTSP-only 连接，自动尝试多种常见流路径"""
        # 常见 RTSP 路径（按常见程度排序）
        paths_to_try = [
            self.config.rtsp_path,      # 配置的路径（如 /stream1）
            "/Streaming/Channels/101",  # 海康威视
        ...
        for path in unique_paths:
            try:
                url = f"rtsp://{self.config.username}:{pwd}@{self.config.ip}:{self.config.rtsp_port}{path}"
                print(f"[ONVIF:{self.config.name}] 尝试 RTSP: ...{path}")
                ...
                if cap.isOpened():
                    ...
                    if ret:
                        ...
                        self._status.last_updated = datetime.now()
                        cap.release()
                        # 更新配置中的路径，让后续 get_stream_url 使用正确路径
                        self.config.rtsp_path = path
                        self.event_bus.publish(CameraEvent(
                            event_type=EventType.DEVICE_ONLINE, camera_name=self.config.name,
                            message=f"RTSP 摄像头已连接: {w}x{h} (路径: {path})",
                        ))
                        print(f"[ONVIF:{self.config.name}] RTSP 连接成功 - {w}x{h} (路径: {path})")
                        return True
                    cap.release()
                else:
                    cap.release()
        ...
        self._status.is_connected = False
        self._status.last_error = "所有 RTSP 路径均失败"
        self._status.last_updated = datetime.now()
        print(f"[ONVIF:{self.config.name}] RTSP 流验证失败 (已尝试 {len(unique_paths)} 种路径)")
        return False
