...
from phase3.network.discovery import (
    discover_usb_cameras, discover_network_cameras, PassiveDiscoveryListener
)
from phase3.core.events import EventBus, CameraEvent
from phase3.core.llm import LocalLLMClient, ParsedCommand
...
{Colors.BOLD}可用命令：{Colors.RESET}
  ...
  {Colors.GREEN}/discover_net{Colors.RESET} - 被动监听 ONVIF 心跳包发现摄像头 (用法: /discover_net [秒数])
  {Colors.GREEN}/listen{Colors.RESET}       - 启动后台持续心跳监听，实时发现新上线摄像头
  {Colors.GREEN}/stop_listen{Colors.RESET}  - 停止后台心跳监听
...
{Colors.BLUE}例如："扫描局域网摄像头"、"连接摄像头并拉流"、"截图"、"自动设置"{Colors.RESET}
"""
    ...
    def __init__(self, camera_manager: CameraManager, llm_client, event_bus: EventBus):
        ...
        self._passive_listener: Optional[PassiveDiscoveryListener] = None  # 后台心跳监听器
    ...
    def _handle_slash_command(self, command: str) -> None:
        ...
        command_map = {
            ...
            "/stop_listen": self._cmd_stop_listen,
            "/help":        self._cmd_help,
            "/quit":        self._cmd_quit,
            "/exit":        self._cmd_quit,
        }
    ...
    def _cmd_discover_network(self, arg: str) -> None:
        """被动监听 ONVIF 心跳包发现局域网摄像头，并自动注册到 CameraManager"""
        # 支持自定义监听时长: /discover_net 30
        listen_secs = 15.0
        if arg.strip():
            try:
                listen_secs = float(arg.strip())
            except ValueError:
                self.display_message(f"无效的时长参数 '{arg.strip()}'，使用默认 15s", "warning")

        self.display_message(f"正在被动监听 ONVIF 心跳包 (最长 {listen_secs:.0f}s) ...", "info")
        devices = discover_network_cameras(
            listen_timeout=listen_secs,
            probe_timeout=5.0,
        )
        if devices:
            self.display_message(f"发现 {len(devices)} 个 ONVIF 摄像头", "success")
            self._last_discovered_devices = devices
            self._register_discovered_devices(devices)
        else:
            self.display_message("未发现 ONVIF 摄像头", "warning")
            self.display_message("提示: 可尝试重启摄像头以触发 Hello 广播，或使用 /listen 持续监听", "info")
            self._last_discovered_devices = []

    def _register_discovered_devices(self, devices) -> None:
        """将扫描发现的设备自动注册到 CameraManager"""
        from phase3.core.config import CameraConfig
    ...
    def _cmd_listen(self, _arg: str) -> None:
        """启动后台持续心跳监听，实时发现新上线的 ONVIF 摄像头"""
        if self._passive_listener and self._passive_listener._thread and self._passive_listener._thread.is_alive():
            self.display_message("后台心跳监听已在运行中", "warning")
            return

        def on_new_device(device):
            print(f"\n{Colors.GREEN}  [心跳] 新设备上线: {device.ip}{Colors.RESET}")
            # 自动注册新发现的设备
            from phase3.core.config import CameraConfig
            cam_name = f"discovered_{device.ip.replace('.', '_')}"
            if cam_name not in self.camera_manager.get_camera_names():
                ...
                self.camera_manager.add_camera(cam_config)
                print(f"{Colors.GREEN}  [自动注册] {cam_name}{Colors.RESET}")

        self._passive_listener = PassiveDiscoveryListener(on_found=on_new_device)
        self._passive_listener.start()
        self.display_message("后台心跳监听已启动，新上线摄像头将自动发现并注册", "success")
        self.display_message("使用 /stop_listen 停止监听", "info")

    def _cmd_stop_listen(self, _arg: str) -> None:
        """停止后台心跳监听"""
        if self._passive_listener:
            self._passive_listener.stop()
            # 汇报已发现的设备
            devices = self._passive_listener.get_devices()
            if devices:
                self.display_message(f"心跳监听停止，共发现 {len(devices)} 个设备", "info")
            else:
                self.display_message("心跳监听停止，未发现设备", "info")
            self._passive_listener = None
        else:
            self.display_message("后台心跳监听未在运行", "warning")
